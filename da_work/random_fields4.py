...
GMSL_variance = GMSL_measure.covariance.matrix(dense=True)[0, 0]
GMSL_std = np.sqrt(GMSL_variance)
print(f"Normalised GMSL std: {GMSL_std:.4f} m")
...
ice_thickness_measure = ice_thickness_measure.affine_mapping(
    operator=sl.ice_projection_operator(
        fp,
...
GMSL_std = np.sqrt(GMSL_variance)
print(f"Normalised GMSL std: {GMSL_std:.4f} m")
print(f"Normalised GMSL mean: {GMSL_measure.expectation[0]:.4f} m")


# %%

slc_from_ice_operator = (
...
sea_level_change_measure = ice_thickness_measure.affine_mapping(
    operator=slc_from_ice_operator,
)

gmsl_from_water_operator = sl.averaging_operator(
    slc_from_ice_operator.codomain,
    [fp.ocean_function / fp.ocean_area],
)

gmsl_calc = sea_level_change_measure.affine_mapping(
    operator=gmsl_from_water_operator,
)
...
altimetry_estimate_operator = sl.averaging_operator(
    sea_surface_height_space,
    [altimetry_projection / altimetry_normalisation],
)
...
posterior_pdf_values = stats.norm.pdf(
    x_axis,
    loc=altimetry_estimate_mean,
    scale=altimetry_estimate_std,
)
