...
for lmax in tqdm(lmaxes):
    ...
    ice_length_scale = 0.1 * 6368000.0
    ice_gmsl_target_std = 0.004
    net_ice_thickness_change = -100.0
    ...
    slc_means.append(_slc_mean)
    slc_stds.append(_slc_std)

    _ssh_mean = _ssh_gmsl.expectation[0]
    _ssh_std = np.sqrt(_ssh_gmsl.covariance.matrix(dense=True)[0, 0])
    ssh_means.append(_ssh_mean)
    ssh_stds.append(_ssh_std)
...
df.to_csv(
    "work/6-lmax_issues/outputs/explore_lmax/error_space_raw.csv",
    index=False,
)
