...
satellite_range = np.round(
    np.linspace(1, 90, resolution), 2
)
...
for (
    load_rad
) in load_radius:  # Added outer loop for load_radius
...
err_r10 = difference_pivot.xs(
    10, level="load_radius", axis=1
)
...
if len(satellite_range) <= max_ticks + 1:
    ...
    x_tick_indices = list(
        range(0, len(satellite_range), step)
    )
