...
This module performs Bayesian inversion of ice thickness change from
...
def generate_grid_cell_weighting_functions(
    ...
    latitude_max : float
        Maximum latitude for altimetry coverage.
    latitude_min : float
        Minimum latitude for altimetry coverage.

    Returns
    -------
    list
        List of weighting functions for each grid cell.
    list
        List of (lat, lon) tuples for grid cell centers.
    ...
    lon_edges = np.arange(
        -180.0, 180.0 + grid_spacing_deg, grid_spacing_deg
    )
...
def setup_gridded_altimetry_inversion(
    ...
    model_length_scale_km : float
        Model length scale in kilometres.
    ice_pointwise_std_m : float
        Pointwise standard deviation for ice thickness change prior (metres).
    ...
    -------
    dict
        Dictionary containing all inversion components.
...
def build_low_degree_preconditioner(
    ...
    """Build preconditioner using lower truncation degree.

    Parameters
    ----------
    lmax_precon : int
        Truncation degree for preconditioner.
    weighting_functions : list
        Grid cell weighting functions (from main problem).
    model_sobolev_order : float
        Sobolev order.
    model_length_scale_km : float
        Length scale in km.
    ice_pointwise_std_m : float
        Ice prior pointwise std in metres.
    measurement_noise_std_m : float
        Measurement noise std in metres.

    Returns
    -------
    LinearOperator
        Inverse of the approximate normal operator for preconditioning.
