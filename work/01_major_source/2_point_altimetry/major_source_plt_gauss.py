...
for mi, gmsl_mean in enumerate(gmsl_target_means):
    for si, gmsl_std in enumerate(gmsl_target_stds):
        ...
        ax.set_title(
            f"Point Altimetry GMSL Estimation Error vs Latitude by Source\n(GMSL Mean: {gmsl_mean}, Std: {gmsl_std})"
        )
        ax.legend()
        ax.grid(alpha=0.3)

        fig.savefig(
            f"figures/major_source_point_gauss_errors_combined_mean_{gmsl_mean}_std_{gmsl_std}.png",
            dpi=600,
        )
        plt.close(fig)
    ...
    for si, gmsl_std in enumerate(gmsl_target_stds):
        for src_i, source in enumerate(sources):
            ...
            fig, (ax1, ax2) = error_latitude_plot(
                ...
                estimate_label="Estimated GMSL (Point)",
                ax1_title=f"{source_labels[source]}: True vs Point Estimated GMSL",
                ax1_ylabel="GMSL (mm)",
                error_mean=err_means_subset,
                error_std=err_stds_subset,
                error_label="Estimation Error",
                ax2_title=f"{source_labels[source]}: Point GMSL Estimation Error",
                ax2_ylabel="Estimation Error (mm)",
                suptitle=f"{source_labels[source]} Point Altimetry GMSL Estimation and Error vs Latitude\n(GMSL Mean: {gmsl_mean}, Std: {gmsl_std})",
                error_100_value=gmsl_std * 1000,
                error_100_value_name=f"1 GMSL Std Dev ({gmsl_std * 1000:.1f} mm)",
            )
            fig.savefig(
                f"figures/major_source_point_gauss_{source}_mean_{gmsl_mean}_std_{gmsl_std}.png",
                dpi=600,
            )
            plt.close(fig)

# %%
# Plot 3: Error std as fraction of signal std
...
ax.set_title(
    "Fractional Point Altimetry GMSL Estimation Error by Source"
)
ax.legend()
ax.grid(alpha=0.3)

fig.savefig(
    "figures/major_source_point_gauss_fractional_error.png",
    dpi=600,
)
plt.close(fig)
