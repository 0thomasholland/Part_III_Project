...
sns.scatterplot(x='lmax', y='mean', data=data_full, ax=ax1, color=color1, label='Mean', s=50)
...
sns.scatterplot(x=data_full['lmax'], y=scaled_std, ax=ax2, color=color2, label='Std Dev (x1e3)', s=50)
...
# Simpler approach:
fig.legend(lines_1 + lines_2, labels_1 + labels_2, loc="upper center", bbox_to_anchor=(0.5, 0.9))
...
p3.set_title("Gaussian Distributions (only by standard deviation, ignores mean) by lmax")
p3.set_xlabel("Value")
