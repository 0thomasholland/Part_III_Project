...
Bayesian inversion is the specific application of this framework to inverse problems, where, rather than seeking a single best-fit solution, it characterizes the entire probability distribution of unknown physical parameters from noisy data. To summarise in a simple case, if we state our prior knowldege of a model space in terms of a Gaussian measure given as:
...
Using the form of Bayes' theorem implemented in \textcite{heathcoteScaleableBayesianFramework2026} originally presented in \textcite{stuartInverseProblemsBayesian2010}, the posterior measure of our updated view of the model space given an observation of the data space would be:

\begin{align}
	m_p = \sim \mathcal{N}(\bar{m}+K() TKTKTK eq33 -> 35
\end{align}
...
There is also a subtle distinction to be made between restricting priors based on knowledge of the physical system and behaviours, as described using informative priors, and restricting , such as using artificial districisation schemes. The former is defensible within a Baysean framework, however the latter is a restriction imposed by compuational or mathematical restriction, and, as discussed in \textcite{stuartInverseProblemsBayesian2010}, can result in artefacts or overly constrained posteriors due to this discretisation.
 Recently, \textcite{coulsonInvertingSeaSurface2026} have used baysiean inversion however have used a discrete TKTK, the method outlined in this report in arguably an improvement of their method, as results in a TKTK.
